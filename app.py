...
from param import PY2
from param import __appname__
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        export = action(
            ...
            shortcuts["open_next"],
            "export",
            self.tr("Export labels to different file"),
            enabled = False,
        )
        ...
        ##########
        selectLabelModel = QtWidgets.QWidgetAction(self)
        selectLabelModel.setDefaultWidget(QtWidgets.QWidget())
        selectLabelModel.defaultWidget().setLayout(QtWidgets.QVBoxLayout())
        #
        selectLabelModelLabel = QtWidgets.QLabel(self.tr("Label"))
        selectLabelModelLabel.setFixedWidth(200)
        selectLabelModelLabel.setAlignment(QtCore.Qt.AlignCenter)
        selectLabelModel.defaultWidget().layout().addWidget(selectLabelModelLabel)
        #
        self._selectLabelModelComboBox = QtWidgets.QComboBox()
        selectLabelModel.defaultWidget().layout().addWidget(self._selectLabelModelComboBox)
            
            
        for i in range(self.uniqLabelList.count()):
            label_i = self.uniqLabelList.item(i).text()
            print("========selectLabelModel========", label_i)
                    
        model_names = []
        self._selectLabelModelComboBox.addItems(model_names)

        # model_index = 0
        # self._selectLabelModelComboBox.setCurrentIndex(model_index)
        # self._selectLabelModelComboBox.currentIndexChanged.connect(
        #     lambda: self.canvas.initializeAiModel(
        #         name=self._selectLabelModelComboBox.currentText()
        #     )
        #     if self.canvas.createMode in ["ai_polygon", "ai_mask"]
        #     else None
        # )
    ...
    def _update_item(self, item, text, flags, group_id, description):
        ...
        self.setDirty()
        ## 感觉下面更新label没用，实际上面已经实现过_update_shape_color
    ...
    def addLabel(self, shape):
        ...
        if self.uniqLabelList.findItemByLabel(shape.label) is None:
            ...
            rgb = self._get_rgb_by_label(shape.label)
            self.uniqLabelList.setItemLabel(item, shape.label, rgb)
            # add Label list first
            self._selectLabelModelComboBox.addItems([text])
    ...
    def _get_rgb_by_label(self, label):
        if self._config["shape_color"] == "auto":
            ...
            if item is None:
                ...
                self.uniqLabelList.setItemLabel(item, label, rgb)
                # update Label list
                self._selectLabelModelComboBox.addItems([label])
    ...
    def newShape(self):
        ...
        if self._config["display_label_popup"] or not text:
            ...
            # 通过当前下拉框的选项确定Label
            text = self._selectLabelModelComboBox.currentText()
            if 0 == self._selectLabelModelComboBox.count():
                text, flags, group_id, description = self.labelDialog.popUp(text)
            else:
                text = self._selectLabelModelComboBox.currentText()
